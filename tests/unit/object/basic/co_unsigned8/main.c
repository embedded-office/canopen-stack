...
void test_size_unknown(void)
{
    CO_NODE  AppNode = { 0 };
    uint8_t  data = 0x11;
    CO_OBJ   Obj  = { CO_KEY(0, 0, CO_OBJ_____RW), CO_TUNSIGNED8, (CO_DATA)(&data)};
    uint32_t size;
...
{
    CO_NODE AppNode = { 0 };
    CO_OBJ  Obj = { CO_KEY(0, 0, CO_OBJ_D___RW), CO_TUNSIGNED8, (CO_DATA)(0x22)};
    uint8_t var;
    CO_ERR  err;
...
void test_read_nodeid(void)
{
    CO_NODE AppNode = { 0 };
    uint8_t data = 0x33;
    CO_OBJ  Obj = { CO_KEY(0, 0, CO_OBJ__N__RW), CO_TUNSIGNED8, (CO_DATA)(&data)};
    uint8_t var;
    CO_ERR  err;
...
{
    uint8_t var = 0x44;
    CO_OBJ  Obj = { CO_KEY(0, 0, CO_OBJ_D___RW), CO_TUNSIGNED8, (CO_DATA)(0x11)};
    CO_ERR  err;
...
void test_write_direct(void)
{
    ...
    TEST_CHECK(err == CO_ERR_NONE);
    TEST_CHECK((uint8_t)Obj.Data == 0x33);
}
