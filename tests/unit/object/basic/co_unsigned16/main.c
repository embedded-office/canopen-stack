...
void test_size_unknown(void)
{
    ...
    CO_OBJ   Obj  = { CO_KEY(0, 0, CO_OBJ_____RW), CO_TUNSIGNED16, (CO_DATA)(&data)};
    uint32_t size;
...
{
    CO_NODE  AppNode = { 0 };
    CO_OBJ   Obj = { CO_KEY(0, 0, CO_OBJ_D___RW), CO_TUNSIGNED16, (CO_DATA)(0x2233)};
    uint16_t var;
    CO_ERR   err;
...
void test_read_nodeid(void)
{
    ...
    CO_OBJ   Obj = { CO_KEY(0, 0, CO_OBJ__N__RW), CO_TUNSIGNED16, (CO_DATA)(&data)};
    uint16_t var;
    CO_ERR   err;
...
{
    uint16_t var = 0x4455;
    CO_OBJ   Obj = { CO_KEY(0, 0, CO_OBJ_D___RW), CO_TUNSIGNED16, (CO_DATA)(0x1122)};
    CO_ERR   err;

    err = COObjRdValue(&Obj, NULL, &var, sizeof(var));

    TEST_CHECK(err == CO_ERR_BAD_ARG);
    TEST_CHECK(var == 0x4455);
}
...
void test_write_direct(void)
{
    ...
    TEST_CHECK(err == CO_ERR_NONE);
    TEST_CHECK((uint16_t)Obj.Data == 0x3344);
}
...
void test_write_pdo_async(void)
{
    CO_NODE  AppNode = { 0 };
    uint16_t data = 0x3344;
    CO_OBJ   Obj = { CO_KEY(0, 0, CO_OBJ___APRW), CO_TUNSIGNED16, (CO_DATA)(&data)};
    uint16_t var = 0x4455;
    CO_ERR   err;
...
void test_write_pdo(void)
{
    CO_NODE  AppNode = { 0 };
    uint16_t data = 0x3344;
    CO_OBJ   Obj = { CO_KEY(0, 0, CO_OBJ____PRW), CO_TUNSIGNED16, (CO_DATA)(&data)};
    uint16_t var = 0x4455;
    CO_ERR   err;
