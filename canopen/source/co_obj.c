...
CO_ERR COObjRdDirect(struct CO_OBJ_T *obj, void *val, uint32_t len)
{
    ...
    if (CO_IS_DIRECT(obj->Key) != 0) {
        if (len == CO_BYTE) {
            *((uint8_t *)val) = (uint8_t)((obj->Data.Direct)&0xff);
        } else if (len == CO_WORD) {
            *((uint16_t *)val) = (uint16_t)((obj->Data.Direct)&0xffff);
        } else if (len == CO_LONG) {
            *((uint32_t *)val) = (uint32_t)(obj->Data.Direct);
        } else {
            result = CO_ERR_BAD_ARG;
        }
    } else {
        sz = CO_GET_SIZE(obj->Key);
        if (obj->Data.Indirect == 0) {
            result = CO_ERR_OBJ_ACC;
        } else if (len == CO_BYTE) {
            if (sz == CO_BYTE) {
                *((uint8_t *)val) = (uint8_t)(*((uint8_t*)(obj->Data.Indirect))&0xff);
            } else if (sz == CO_WORD) {
                *((uint8_t *)val) = (uint8_t)(*((uint16_t*)(obj->Data.Indirect))&0xff);
            } else if (sz == CO_LONG) {
                *((uint8_t *)val) = (uint8_t)(*((uint32_t*)(obj->Data.Indirect))&0xff);
            } else {
                result = CO_ERR_BAD_ARG;
            }
        } else if (len == CO_WORD) {
            if (sz == CO_BYTE) {
                *((uint16_t *)val) = (uint16_t)(*((uint8_t*)(obj->Data.Indirect))&0xffff);
            } else if (sz == CO_WORD) {
                *((uint16_t *)val) = (uint16_t)(*((uint16_t*)(obj->Data.Indirect))&0xffff);
            } else if (sz == CO_LONG) {
                *((uint16_t *)val) = (uint16_t)(*((uint32_t*)(obj->Data.Indirect))&0xffff);
            } else {
                result = CO_ERR_BAD_ARG;
            }
        } else if (len == CO_LONG) {
            if (sz == CO_BYTE) {
                *((uint32_t *)val) = (uint32_t)(*((uint8_t*)(obj->Data.Indirect)));
            } else if (sz == CO_WORD) {
                *((uint32_t *)val) = (uint32_t)(*((uint16_t*)(obj->Data.Indirect)));
            } else if (sz == CO_LONG) {
                *((uint32_t *)val) = (uint32_t)(*((uint32_t*)(obj->Data.Indirect)));
            } else {
                result = CO_ERR_BAD_ARG;
            }
        }
...
CO_ERR COObjWrDirect(CO_OBJ *obj, void *val, uint32_t len)
{
    ...
    if (CO_IS_DIRECT(obj->Key) != 0) {
        if (len == CO_BYTE) {
            obj->Data.Direct = (uint32_t)(*((uint8_t*)val)&0xff);
        } else if (len == CO_WORD) {
            obj->Data.Direct = (uint32_t)(*((uint16_t*)val)&0xffff);
        } else if (len == CO_LONG) {
            obj->Data.Direct = (uint32_t)(*((uint32_t *)val));
        } else {
            result = CO_ERR_BAD_ARG;
        }
    } else {
        ...
        if (obj->Data.Indirect == 0) {
            ...
            if (len == CO_BYTE) {
                *((uint8_t *)(obj->Data.Indirect)) = (uint8_t)(*((uint8_t*)(val))&0xff);
            } else if (len == CO_WORD) {
                *((uint8_t *)(obj->Data.Indirect)) = (uint8_t)(*((uint16_t*)(val))&0xff);
            } else if (len == CO_LONG) {
                *((uint8_t *)(obj->Data.Indirect)) = (uint8_t)(*((uint32_t*)(val))&0xff);
            } else {
                result = CO_ERR_BAD_ARG;
            }
        } else if (sz == CO_WORD) {
            if (len == CO_BYTE) {
                *((uint16_t *)(obj->Data.Indirect)) = (uint16_t)(*((uint8_t*)(val))&0xffff);
            } else if (len == CO_WORD) {
                *((uint16_t *)(obj->Data.Indirect)) = (uint16_t)(*((uint16_t*)(val))&0xffff);
            } else if (len == CO_LONG) {
                *((uint16_t *)(obj->Data.Indirect)) = (uint16_t)(*((uint32_t*)(val))&0xffff);
            } else {
                result = CO_ERR_BAD_ARG;
            }
        } else if (sz == CO_LONG) {
            if (len == CO_BYTE) {
                *((uint32_t *)(obj->Data.Indirect)) = (uint32_t)(*((uint8_t*)(val)));
            } else if (len == CO_WORD) {
                *((uint32_t *)(obj->Data.Indirect)) = (uint32_t)(*((uint16_t*)(val)));
            } else if (len == CO_LONG) {
                *((uint32_t *)(obj->Data.Indirect)) = (uint32_t)(*((uint32_t*)(val)));
            } else {
                result = CO_ERR_BAD_ARG;
            }
        }
...
uint32_t COTypeStringSize(struct CO_OBJ_T *obj, struct CO_NODE_T *node, uint32_t width)
{
    ...
    (void)node;
    (void)width;
    str = (CO_OBJ_STR *)(obj->Data.Indirect);
    ptr = str->Start;
    while (*ptr != '\0') {
        strlen++;
        ptr++;
...
uint32_t COTypeDomainSize(struct CO_OBJ_T *obj, struct CO_NODE_T *node, uint32_t width)
{
    ...
    dom = (CO_OBJ_DOM *)(obj->Data.Indirect);
