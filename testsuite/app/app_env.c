...
static char         CU_location[32];                  /*!< Filtered filename for unity            */
...
void TS_FatalError(char const *file, uint32_t line)
{
    TS_Printf("Fatal Error in '%s' at line #%d\n", file, line);

    TS_Fail("Abort test.", line);
}
...
void TS_CreateMandatoryDir(void)
{
    ...
    ODAdd(&TS_ODDyn, CO_KEY(0x1000, 0, CO_UNSIGNED32|CO_OBJ_D__R_), 0, 0x00000000);
    ODAdd(&TS_ODDyn, CO_KEY(0x1001, 0, CO_UNSIGNED8 |CO_OBJ___PR_), 0, (uint32_t)&TS_ErrReg);
    ODAdd(&TS_ODDyn, CO_KEY(0x1005, 0, CO_UNSIGNED32|CO_OBJ_D__R_), 0, 0x00000080);
    ODAdd(&TS_ODDyn, CO_KEY(0x1017, 0, CO_UNSIGNED16|CO_OBJ____RW), 0, (uint32_t)&COTNmtHbProd);
    ODAdd(&TS_ODDyn, CO_KEY(0x1018, 0, CO_UNSIGNED8 |CO_OBJ_D__R_), 0, 0x04);
    ODAdd(&TS_ODDyn, CO_KEY(0x1018, 1, CO_UNSIGNED32|CO_OBJ_D__R_), 0, 0x00000000);
    ODAdd(&TS_ODDyn, CO_KEY(0x1018, 2, CO_UNSIGNED32|CO_OBJ_D__R_), 0, 0x00000000);
    ODAdd(&TS_ODDyn, CO_KEY(0x1018, 3, CO_UNSIGNED32|CO_OBJ_D__R_), 0, 0x00000000);
    ODAdd(&TS_ODDyn, CO_KEY(0x1018, 4, CO_UNSIGNED32|CO_OBJ_D__R_), 0, 0x00000000);
    for (srv = 0; srv < CO_SDOS_N; srv++) {
        ODAdd(&TS_ODDyn, CO_KEY(0x1200+srv, 0, CO_UNSIGNED8 |CO_OBJ_D__R_), 0, 0x02);
        ODAdd(&TS_ODDyn, CO_KEY(0x1200+srv, 1, CO_UNSIGNED32|CO_OBJ_DN_R_), 0, 0x00000600+(srv*0x10));
        ODAdd(&TS_ODDyn, CO_KEY(0x1200+srv, 2, CO_UNSIGNED32|CO_OBJ_DN_R_), 0, 0x00000580+(srv*0x10));
    }
...
{
    TS_ODAdd(CO_KEY(0x1800 + num, 0, CO_UNSIGNED8 |CO_OBJ_D__R_), 0,         (uint32_t)0x05);
    TS_ODAdd(CO_KEY(0x1800 + num, 1, CO_UNSIGNED32|CO_OBJ__N_R_), 0,         (uint32_t)id);
    TS_ODAdd(CO_KEY(0x1800 + num, 2, CO_UNSIGNED8 |CO_OBJ____R_), 0,         (uint32_t)type);
    TS_ODAdd(CO_KEY(0x1800 + num, 3, CO_UNSIGNED16|CO_OBJ____R_), 0,         (uint32_t)inhibit);
    TS_ODAdd(CO_KEY(0x1800 + num, 5, CO_UNSIGNED16|CO_OBJ____RW), CO_TEVENT, (uint32_t)evtimer);
}
...
void TS_CreateTPdoMap(uint8_t num, uint32_t *map, uint8_t *len)
{
    ...
    TS_ODAdd(CO_KEY(0x1A00 + num, 0, CO_UNSIGNED8 |CO_OBJ____R_), 0, (uint32_t)len);
    if (*len != 0) {
        for (n = 0; n < *len; n++) {
            TS_ODAdd(CO_KEY(0x1A00 + num, 1 + n, CO_UNSIGNED32|CO_OBJ____R_), 0, (uint32_t)&map[n]);
        }
    }
}
...
{
    TS_ODAdd(CO_KEY(0x1400 + num, 0, CO_UNSIGNED8 |CO_OBJ_D__R_), 0, (uint32_t)0x02);
    TS_ODAdd(CO_KEY(0x1400 + num, 1, CO_UNSIGNED32|CO_OBJ__N_R_), 0, (uint32_t)id);
    TS_ODAdd(CO_KEY(0x1400 + num, 2, CO_UNSIGNED8 |CO_OBJ____R_), 0, (uint32_t)type);
}
...
void TS_CreateRPdoMap(uint8_t num, uint32_t *map, uint8_t *len)
{
    ...
    TS_ODAdd(CO_KEY(0x1600 + num, 0, CO_UNSIGNED8 |CO_OBJ____R_), 0, (uint32_t)len);
    if (*len != 0) {
        for (n = 0; n < *len; n++) {
            TS_ODAdd(CO_KEY(0x1600 + num, 1 + n, CO_UNSIGNED32|CO_OBJ____R_), 0, (uint32_t)&map[n]);
        }
    }
}
...
{
    TS_ODAdd(CO_KEY(0x1800 + num, 0, CO_UNSIGNED8 |CO_OBJ_D__R_), 0,           (uint32_t)0x02);
    TS_ODAdd(CO_KEY(0x1800 + num, 1, CO_UNSIGNED32|CO_OBJ__N_RW), CO_TPDOID,   (uint32_t)id);
    TS_ODAdd(CO_KEY(0x1800 + num, 2, CO_UNSIGNED8 |CO_OBJ____RW), CO_TPDOTYPE, (uint32_t)type);
    TS_ODAdd(CO_KEY(0x1800 + num, 3, CO_UNSIGNED16|CO_OBJ____RW), 0,           (uint32_t)inhibit);
    TS_ODAdd(CO_KEY(0x1800 + num, 5, CO_UNSIGNED16|CO_OBJ____RW), CO_TEVENT,   (uint32_t)evtimer);
}
...
void TS_CreateDynTPdoMap(uint8_t num, uint32_t *map, uint8_t *len)
{
    ...
    TS_ODAdd(CO_KEY(0x1A00 + num, 0, CO_UNSIGNED8 |CO_OBJ____RW), CO_TPDONUM, (uint32_t)len);
    if (*len != 0) {
        for (n = 0; n < *len; n++) {
            TS_ODAdd(CO_KEY(0x1A00 + num, 1 + n, CO_UNSIGNED32|CO_OBJ____RW), CO_TPDOMAP, (uint32_t)&map[n]);
        }
    }
}
...
{
    TS_ODAdd(CO_KEY(0x1400 + num, 0, CO_UNSIGNED8 |CO_OBJ_D__R_), 0,           (uint32_t)0x02);
    TS_ODAdd(CO_KEY(0x1400 + num, 1, CO_UNSIGNED32|CO_OBJ__N_RW), CO_TPDOID,   (uint32_t)id);
    TS_ODAdd(CO_KEY(0x1400 + num, 2, CO_UNSIGNED8 |CO_OBJ____RW), CO_TPDOTYPE, (uint32_t)type);
}
...
void TS_CreateDynRPdoMap(uint8_t num, uint32_t *map, uint8_t *len)
{
    ...
    TS_ODAdd(CO_KEY(0x1600 + num, 0, CO_UNSIGNED8 |CO_OBJ____RW), CO_TPDONUM, (uint32_t)len);
    if (*len != 0) {
        for (n = 0; n < *len; n++) {
            TS_ODAdd(CO_KEY(0x1600 + num, 1 + n, CO_UNSIGNED32|CO_OBJ____RW), CO_TPDOMAP, (uint32_t)&map[n]);
        }
    }
}
...
{
    char *ptr = (char *)filename;                     /* Local: pointer in unfiltered filename    */
    char *str = CU_location;                          /* Local: pointer in result filename        */
                                                      /*------------------------------------------*/
    while (*ptr != 0) {                               /* goto end of string                       */
        ptr++;
    }
                                                      /*------------------------------------------*/
    ...
    ptr++;                                            /* move behind file delimiter               */
                                                      /*------------------------------------------*/
    while ((*ptr != ' ') && (*ptr != 0)) {            /* copy loop till end of string or space    */
        *str = *ptr;
        str++;
        ptr++;
    }
    *str = 0;                                         /* ensure correct end of string             */
                                                      /*------------------------------------------*/
    return CU_location;                               /* return filtered filename result          */
}

/*! \brief   Global Test Setup
*
*   This function is called before each individual test case is executed.
*/
void setUp(void)
{
...
/*! \brief   Global Test Setup
*
*   This function is called after each individual test case is executed with
*   or without an error or ignored.
*/
void tearDown(void)
{
    TS_CallbackDeInit();
