...
TS_DEF_MAIN(TS_BlkWr_ObjNotExist)
{
    ...
    CHK_ERR(&node, CO_ERR_OBJ_NOT_FOUND);             /* check for expected error                 */
}
...
TS_DEF_MAIN(TS_BlkWr_DomainNullPtr)
{
    ...
    uint8_t   sub  = 0;
                                                      /*------------------------------------------*/
    TS_CreateMandatoryDir();
    TS_ODAdd(CO_KEY(idx, sub, CO_DOMAIN|CO_OBJ____RW), CO_TDOMAIN, CO_DATA_SET_VAL(0));
    TS_CreateNode(&node,0);
...
TS_DEF_MAIN(TS_BlkWr_ExpWrAfter43ByteDomain)
{
    ...
    TS_CreateMandatoryDir();
    dom = DomCreate(idx, sub, CO_OBJ____RW, size);
    TS_ODAdd(CO_KEY(idx2, sub2, CO_UNSIGNED32|CO_OBJ____RW), 0, CO_DATA_SET_REF(&val));
    TS_CreateNode(&node,0);
